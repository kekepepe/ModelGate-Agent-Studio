...
def _agent_for(db: Session, capability: str):
    roles = {"research": "research", "code_edit": "coder", "review": "reviewer", "direct": "summarizer"}
...
def plan_goal(db: Session, goal: Goal) -> List[Task]:
    ...
    for index, spec in enumerate(specs):
        agent = (_agent_for(db, "direct") if spec["capability"] == "planner" else _agent_for(db, spec["capability"]))
        if not agent:
            continue
