...
def test_frontend_backend_goal_creates_parallel_isolated_tasks(db_session):
    for role in ("planner", "coder", "reviewer"):
        db_session.add(AgentStation(id=str(uuid.uuid4()), name=role, role=role, default_model_id=f"{role}-model", is_enabled=True))
    ...
    assert set(verifier._get_json("dependencies")) == {task.id for task in coding}
